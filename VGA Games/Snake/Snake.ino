/*
  ...
  with sound
  Arduino IDE 1.6.4 
  Version 1.3 (December the 13th 2016)
...
#define BUTTON_1 12 //digital 
#define BUTTON_2 11 //digital 
#define BUTTON_3 10 //digital 
#define BUTTON_4 13 //digital 
...
int foodX = 60; 
int foodY = 30;
byte sx[51]; // (scoreMax + 1)*slength + 1
byte sy[51];
byte slength = 3;
byte delta = 5; 
int wleft = 36; 
...
void loop() {
  //Serial.println(VGAX_WIDTH + " by " + VGAX_HEIGHT);
...
//--------------------- check eating food -----------------------------------
     ...
     //if (vga.getpixel(int(sx[i]), int(sy[i])) == 1 ){
        toneSafe(660,30);  
        foodIni(); 
        drawBorder(); 
        vga.putpixel(foodX, foodY, 1);
